...
def get_phone_urls(init_url):
    ...
    total_phone_pages = 20
    while next_page_url and total_phone_pages:
        print 'current working on: ', next_page_url
...
def single_page_urls(url):
    ...
    #todo: here I just hard code the next_page_url part....
    ori_phone_index = int(url[url.find("&s=")+len("&s="): url.find("&size=")])
    new_phone_index = ori_phone_index + ONE_PAGE_NUM
    ori_substr = "&s=%d" % ori_phone_index
    new_substr = "&s=%d" % new_phone_index
    next_page_url = url.replace(ori_substr, new_substr)
    return temp_urls, next_page_url


def test_single_page_urls(url):
    single_page = get_html(url, proxy=PROXY, cache=CACHE)
    print single_page
...
def main():
    # test_get_phone_urls()
    # test_url = "http://s.etao.com/search?cat=50080001&usearch=yes&mt=778209&pn=36&s=648&size=36#J_FilterBar"
    # test_single_page_urls(test_url)
    init_url = config.get('crawler', 'init_url')
